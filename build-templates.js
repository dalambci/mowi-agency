      ...
      <div class="footer-panel">
        <div class="footer-top">
          <div class="footer-brand">
            ...
            <p class="footer-tagline">Werk uit handen, niet uit het oog</p>
...
function renderCanvasInteractive(template, fitOverride) {
  ...
  return `<div class="wf-canvas" data-wf-canvas data-wf-fit="${fitOverride || (graph.width < 400 ? "center" : "fit")}" role="group" aria-label="${esc(template.label)}">
...
/* ---------------------------------------------------------------------------
 * The demo sections pair the live demo card with a REAL workflow canvas — the
 * same pannable component the template detail pages use, not a picture of one.
 *
 * It is generated here rather than hand-pasted into three pages because it is
 * eight absolutely-positioned nodes plus hand-computed SVG edge paths that have
 * to stay in step with content/templates/templates.json. Hand-copies would rot
 * silently the first time a template changed. A rebuild now updates every page
 * that shows one.
 *
 * Framing note: these graphs are 520x1272, tall and narrow. "fit" would shrink
 * all of that into a card-sized box and the node labels become unreadable
 * confetti — which is exactly what the reference screenshot does, and is not
 * worth copying. "center" opens at 1:1 so the nodes stay legible and the
 * visitor drags to see the rest, which is also the more convincing gesture:
 * it proves the thing is real rather than a screenshot.
 * ------------------------------------------------------------------------- */
const DEMO_WORKFLOW_MARK = {
  start: "<!-- wf-demo:start — generated by build-templates.js, do not hand-edit -->",
  end: "<!-- wf-demo:end -->",
};

// The type tabs sit ABOVE both cards (Sal, 2026-09-16, matching
// elevenlabs.io/voice-agents), while the workflow panels they switch live
// inside the right-hand card — so the generated markup lands in two places on
// the page and needs its own marker pair.
const DEMO_TABS_MARK = {
  start: "<!-- wf-tabs:start — generated by build-templates.js, do not hand-edit -->",
  end: "<!-- wf-tabs:end -->",
};
...
function injectDemoWorkflows(templates) {
  const body = DEMO_WORKFLOW_TYPES.map((t, i) => {
    ...
    const sel = i === 0;
    return {
      // data-demo-type/-company/-sub are what js/demo.js reads to retitle the
      // LEFT card and to tell the dashboard which agent to mint. The company
      // name is duplicated from the dashboard's config/demo_profiles.php —
  ...
  const tabsMarkup = `        <div class="demo-seg demo-type-seg" role="tablist" aria-label="Soort gesprek" data-demo-types data-wf-refit>
          ${body.map((b) => b.tab).join("\n          ")}
        </div>
`;

  const panelsMarkup = `${body.map((b) => b.panel).join("\n\n            ")}
`;
  ...
  DEMO_WORKFLOW_PAGES.forEach((file) => {
    ...
    const replaceBlock = (mark, markup, indent) => {
      const from = html.indexOf(mark.start);
      const to = html.indexOf(mark.end);
      if (from === -1 || to === -1) return false;
      html = html.slice(0, from) + mark.start + "\n" + markup + indent + html.slice(to);
      return true;
    };

    // Both blocks or neither — a page with tabs but no panels (or the other
    // way round) renders a control that switches nothing.
    const hasTabs = replaceBlock(DEMO_TABS_MARK, tabsMarkup, "        ");
    const hasPanels = replaceBlock(DEMO_WORKFLOW_MARK, panelsMarkup, "            ");

    if (hasTabs !== hasPanels) {
      throw new Error(`build-templates: ${file} carries only one of the two demo markers (tabs: ${hasTabs}, panels: ${hasPanels})`);
    }

    if (hasTabs && html !== before) {
      fs.writeFileSync(full, html);
      written += 1;
    }
  });

  console.log(`Refreshed the demo type tabs + workflow panels (${DEMO_WORKFLOW_TYPES.length} types) on ${written} page(s).`);
}
