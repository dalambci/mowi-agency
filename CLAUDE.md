...
- `templates.html` and every `templates/<slug>.html` are **generated by `build-templates.js`**
  ...
  versioned together by `TPL_ASSET_VERSION` in the generator (**`20260906-4`**); `style.css`/
...
- Detail pages load `js/workflow-canvas.js` (`WF_JS_VERSION` in the generator, `20260906-1`).
  Since 2026-09-06 it honours `data-wf-fit`, ported from the dashboard's copy: `"fit"` scales the
  whole graph into the frame above the hint pill (the zoom floor follows the fit scale, so zooming
  out stops at the fitted view), anything else opens at 1:1 centred and pans. The generator writes
  `fit` for graphs 400px and wider and `center` for the narrow single-column ones, the dashboard's
  own compact/full rule. Before that day the attribute was written but ignored.
- **Detail pages mirror the dashboard's, one `renderDetailPage()` for all three kinds (2026-09-06,
  ...
  suggest barber templates to a hotel"): agents first, then workflows, then dashboards, four at
  most — `relatedFor()` here, `FlowTemplateController::relatedTo()` there. Website-only, on
  purpose: the CTA is signup, so the dashboard's "what the Builder receives" box is not mirrored.
  The toolbar on `/templates` lost its two hairlines the same evening, and a section divider is
  drawn only between two visible sections (`.tpl-section:not([hidden]) ~ .tpl-section:not([hidden])`).
  **Superseded the same day, kept for history:** for a few hours the agent pages were per-branche
  landing pages (`renderAgentDetailPage()`, the tagline as a display headline, onboarding steps
  read out of `call-agent.html` by a `readAgentSteps()`); git history has it, nothing reads the
  landing pages at build time any more. `qa-templates.js` section 10 pins the kapper page,
  including "every node inside the frame" and "related cards all carry kapper". The same
  evening `css/workflow-canvas.css` got the dashboard's `.wf-hint{width:max-content}` fix (the
