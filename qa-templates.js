...
async function run(browserType, label, viewport, device) {
  ...
  check(`${label} dashboard pictures show their koppelingen, not a glyph`,
    art.charts === 0 && art.dashLogoRows === d, `charts ${art.charts} logo rows ${art.dashLogoRows} of ${d}`);
  check(`${label} up to four logo tiles per picture, never more`,
    art.maxTiles > 0 && art.maxTiles <= 4, String(art.maxTiles));
